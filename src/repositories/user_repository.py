...
def check_password(username: str, password: str) -> bool:
    ...
    result = database.session.execute(text(sql), {"username": username})
...
def set_show_hidden(new_show_hidden: bool) -> None:
    ...
    database.session.execute(
        text(sql), {"user_id": session["user_id"], "show_hidden": new_show_hidden}
    )
...
def add_user(username: str, password: str) -> int:
    ...
    result = database.session.execute(
        text(sql), {"username": username, "password": password_hash}
    )
    database.session.commit()
